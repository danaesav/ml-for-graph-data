from baseline_models import GCN, GatedGCNNet, H2GCN, GCN_LPA, SAGE_sup, LANC, GAT, MLP
...
def model_train():
    ...
    print("check the size of train data: ", output[train_mask].shape)
    print("check the size of val data: ", output[val_mask].shape)
    print("check the size of test data: ", output[test_mask].shape)
...
if __name__ == "__main__":
    ...
    if args.data_name == "pcg":
        ...
        ########################### for ablation study use all nodes for training #############################
    ...
    if args.model_name.startswith("GCNLPA"):
        ...
        model = H2GCN(nfeat=G.x.shape[1], nhid=hidden_dim, nclass=G.y.shape[1], dropout=0.5, multi_class=args.multi_class)
    ...
    path_to_checkpoints_folder = "../Data_Hardness/checkpoints_" + args.data_name + "_" + args.split_name + "_"+ args.model_name

    if not os.path.exists(path_to_checkpoints_folder):
        os.makedirs(path_to_checkpoints_folder)
    #######################################################################################

    optimizer = torch.optim.Adam(model.parameters(), lr=args.learning_rate, weight_decay=args.weight_decay)
    early_stopping = EarlyStopping(path_to_checkpoints_folder, model_name=args.model_name, split_name=args.split_name, patience=args.patience, verbose=True)
    ...
    if args.model_name.startswith("SAGE"):
        for epoch in range(1, args.epochs):
            ...
            early_stopping(path_to_checkpoints_folder, loss_val, model, epoch=str(epoch))
        ...
        saved_checkpoints = glob.glob(os.path.join(path_to_checkpoints_folder + "/" + args.model_name + "___" + args.split_name + "_" + '_checkpoint_', '*'))
        saved_checkpoints.sort(key=os.path.getmtime)
        last_checkpoint = saved_checkpoints[-1]
    ...
    ############################################################## other models #################################################
        ...
        print("Optimization Finished!")
        # load the last checkpoint with the best model
        #model.load_state_dict(torch.load(path_to_checkpoints_folder + "/" + args.model_name + "___" + args.split_name + '_checkpoint.pt'))
        saved_checkpoints = glob.glob(os.path.join(path_to_checkpoints_folder + "/" + args.model_name + "___" + args.split_name + "_" + '_checkpoint_', '*'))
        saved_checkpoints.sort(key=os.path.getmtime)
        last_checkpoint = saved_checkpoints[-1]
        model.load_state_dict(torch.load(last_checkpoint))
