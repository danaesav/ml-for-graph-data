####################################################################
from model_reset import FPLPGCN_dw, FPLPGCN_dw_linear, FPLPGCN_dw_MLP
####################################################################
from data_loader_split import load_pcg, load_humloc, load_eukloc, load_yelp, load_blogcatalog, load_hyper_data, load_DBLP, load_cora, load_citeseer
...
if __name__ == "__main__":
    ...
    for epoch in range(args.epochs):
        ...
        # print(y_pad)
        output = model(x, y_pad, edge_index, G.deep_walk_emb, G.label_input_mask)

        ################################# Multi-label Classification #####################################
        if args.multi_class == False:

            # calculate loss on the supervision nodes only
            loss = BCE_loss(output[G.supervision_mask], G.y[G.supervision_mask])
