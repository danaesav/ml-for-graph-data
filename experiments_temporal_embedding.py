...
PARAM = {'NUM_NODES' : 3000,  # Must match N in generator config
        ...
        'TEST_RATIO' : 0.2,
        'EMBEDDING_METHOD' : ['Node2Vec', 'Node2Vec Recurrent'],
...
def initialize_models(param):
    ...
    models = {}
    for embmeth in param["EMBEDDING_METHOD"]:
...
def experiment(param, datasets, display = True):
    ...
    model_names = param["EMBEDDING_METHOD"]
...
def plotting(param, results, datasets, zoom = False):
    ...
    for i in range(len(metric_names)):
        ...
        plot_names = [(f'{p} (Train)', f'{p} (Val)') for p in param["EMBEDDING_METHOD"]]
...
def experiment_main(param):
    ...
    for alpha in alphas[::-1]:
        ...
        param["ALPHA"] = alpha
        for embmeth in param["EMBEDDING_METHOD"]:
            dataset_loader = DatasetLoader(config, param["EMBEDDING_DIM"], embmeth, param["FILENAME"], param["BASEFILE"])
